// ==UserScript==
// @name         Gemini De-Mark (CloudWaddie Edition)
// @namespace    https://github.com/cloudwaddie
// @version      1.0.0
// @description  Automated watermark removal for Gemini generated imagery.
// @author       cloudwaddie
// @match        https://gemini.google.com/*
// @icon         https://www.google.com/s2/favicons?domain=gemini.google.com
// @grant        GM_xmlhttpRequest
// @grant        unsafeWindow
// @run-at       document-end
// @license      MIT
// ==/UserScript==

(async () => {
    'use strict';

    // --- Constants & Assets ---
    // Background patterns required for the subtraction algorithm
    const ASSETS = {
        SMALL_48: "data:image/png;base64,",
        LARGE_96: "data:image/png;base64,"
    };

    // --- Core Logic ---
    const CoreProcessor = {
        masks: new Map(),

        // Generate opacity mask from base image data
        createMask: (imageData) => {
            const { width, height, data } = imageData;
            const mask = new Float32Array(width * height);
            for (let i = 0; i < mask.length; i++) {
                // Calculate max channel intensity normalized to 0-1
                const offset = i * 4;
                const max = Math.max(data[offset], data[offset + 1], data[offset + 2]);
                mask[i] = max / 255.0;
            }
            return mask;
        },

        // Clean the image using alpha subtraction
        clean: (targetData, mask, config) => {
            const { x, y, width, height } = config.pos;
            const threshold = 0.002;
            const maxAlpha = 0.99;
            const white = 255;

            for (let r = 0; r < height; r++) {
                for (let c = 0; c < width; c++) {
                    const globalIdx = ((y + r) * targetData.width + (x + c)) * 4;
                    const maskIdx = r * width + c;

                    let alpha = mask[maskIdx];
                    if (alpha < threshold) continue;

                    alpha = (alpha > maxAlpha) ? maxAlpha : alpha;
                    const inverse = 1.0 - alpha;

                    // Restore RGB channels
                    for (let ch = 0; ch < 3; ch++) {
                        const current = targetData.data[globalIdx + ch];
                        // Original pixel reconstruction formula
                        const restored = (current - alpha * white) / inverse;
                        // Clamp values
                        targetData.data[globalIdx + ch] = restored < 0 ? 0 : (restored > 255 ? 255 : Math.round(restored));
                    }
                }
            }
        },

        // Determine watermark configuration based on image resolution
        getConfig: (w, h) => {
            const isHighRes = w > 1024 && h > 1024;
            const size = isHighRes ? 96 : 48;
            const padding = isHighRes ? 64 : 32;
            return {
                size,
                src: isHighRes ? ASSETS.LARGE_96 : ASSETS.SMALL_48,
                pos: {
                    x: w - padding - size,
                    y: h - padding - size,
                    width: size,
                    height: size
                }
            };
        },

        // Main processing pipeline
        run: async (img) => {
            const cvs = document.createElement('canvas');
            cvs.width = img.width;
            cvs.height = img.height;
            const ctx = cvs.getContext('2d');
            ctx.drawImage(img, 0, 0);

            const imageData = ctx.getImageData(0, 0, cvs.width, cvs.height);
            const config = CoreProcessor.getConfig(cvs.width, cvs.height);

            // Lazy load mask
            if (!CoreProcessor.masks.has(config.size)) {
                const maskImg = await Utils.loadImage(config.src);
                const mCvs = document.createElement('canvas');
                mCvs.width = config.size;
                mCvs.height = config.size;
                mCvs.getContext('2d').drawImage(maskImg, 0, 0);
                const mData = mCvs.getContext('2d').getImageData(0, 0, config.size, config.size);
                CoreProcessor.masks.set(config.size, CoreProcessor.createMask(mData));
            }

            CoreProcessor.clean(imageData, CoreProcessor.masks.get(config.size), config);
            ctx.putImageData(imageData, 0, 0);
            return cvs;
        }
    };

    // --- Utilities ---
    const Utils = {
        // Track processed images to avoid infinite loops
        processed: new WeakSet(),

        // Debounce helper
        debounce: (fn, delay) => {
            let timer;
            return (...args) => {
                clearTimeout(timer);
                timer = setTimeout(() => fn(...args), delay);
            };
        },

        // Load image helper
        loadImage: (src) => new Promise((resolve, reject) => {
            const i = new Image();
            i.onload = () => resolve(i);
            i.onerror = reject;
            i.src = src;
        }),

        // Sanitize URL to get max resolution
        cleanUrl: (url) => url.replace(/=s\d+(?=[-?#]|$)/, "=s0"),

        // Identify Gemini generated content
        isTarget: (img) => img.closest("generated-image,.generated-image-container") !== null && img.src.includes("googleusercontent.com"),

        // Fetch wrapper for GM_xmlhttpRequest
        fetchBlob: (url) => new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                method: "GET",
                url: url,
                responseType: "blob",
                onload: (res) => resolve(res.response),
                onerror: reject
            });
        })
    };

    // --- DOM Watcher ---
    const scanAndClean = async () => {
        const images = [...document.querySelectorAll('img')].filter(Utils.isTarget);

        for (const img of images) {
            if (Utils.processed.has(img)) continue;
            Utils.processed.add(img); // Mark as seen immediately

            const originalUrl = img.src;
            try {
                // Fetch full res, process, replace
                const hqBlob = await Utils.fetchBlob(Utils.cleanUrl(originalUrl));
                const hqImg = await Utils.loadImage(URL.createObjectURL(hqBlob));
                const cleanCanvas = await CoreProcessor.run(hqImg);

                cleanCanvas.toBlob(blob => {
                   img.src = URL.createObjectURL(blob);
                   console.log('[CloudWaddie] Cleaned:', originalUrl);
                }, 'image/png');

            } catch (e) {
                console.error('[CloudWaddie] Failed:', e);
                img.src = originalUrl; // Revert on failure
            }
        }
    };

    // --- Initialization & Hooks ---
    const init = () => {
        console.log('[CloudWaddie] Gemini De-Mark Active');

        // 1. DOM Observer
        const observer = new MutationObserver(Utils.debounce(scanAndClean, 150));
        observer.observe(document.body, { childList: true, subtree: true });

        // 2. Network Interceptor (Fetch Proxy)
        const nativeFetch = unsafeWindow.fetch;
        const GEMINI_REGEX = /^https:\/\/lh3\.googleusercontent\.com\/rd-gg(?:-dl)?\/.+=s(?!0-d\?).*/;

        unsafeWindow.fetch = new Proxy(nativeFetch, {
            apply: async function(target, thisArg, args) {
                let url = (typeof args[0] === 'string') ? args[0] : args[0]?.url;

                if (url && GEMINI_REGEX.test(url)) {
                    console.log('[CloudWaddie] Intercepting Request:', url);

                    // Force high res URL
                    const cleanUrl = Utils.cleanUrl(url);
                    if (typeof args[0] === 'string') args[0] = cleanUrl;
                    else if (args[0]?.url) args[0].url = cleanUrl;

                    const response = await Reflect.apply(target, thisArg, args);
                    if (!response.ok) return response;

                    // Process blob before returning to app
                    try {
                        const blob = await response.blob();
                        const img = await Utils.loadImage(URL.createObjectURL(blob));
                        const canvas = await CoreProcessor.run(img);

                        return new Promise(resolve => {
                            canvas.toBlob(newBlob => {
                                resolve(new Response(newBlob, {
                                    status: response.status,
                                    statusText: response.statusText,
                                    headers: response.headers
                                }));
                            });
                        });
                    } catch (e) {
                        return response;
                    }
                }
                return Reflect.apply(target, thisArg, args);
            }
        });

        // Initial scan
        scanAndClean();
    };

    init();
})();
